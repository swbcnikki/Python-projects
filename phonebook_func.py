...
#This is to import our other modules so we can have access to them
import phonebook_main
import phonebook_gui

def center_window(self, w, h): # pass in the tkinter frame (self.master) referece and the w and h
...
def ask_quit(self):
    if messagebox.askokcancel('Exit progam', 'Okay to exit application?'):
        #This closes the app
        self.master.destroy()
        os._exit(0)
...
def create_db(self):
    ...
    with conn:
        ...
        #You must commit() to safe changes & close the database connection
        conn.commit()
    conn.close()
    first_run(self)
...
def first_run(self):
    data = ('John','Doe','John Doe', '555-555-5555','jdoe@noemail.com')
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
...
def onSelect(self,event):
    ...
    with conn:
        ...
        #This returns s tuple and we can slice it into 4 parts using data[] during the iteration
        for data in varBody:
